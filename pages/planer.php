...
if (isset($_GET['page'])) {
 ...
 // Step 1: Handle form submission to update planneds
 if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    foreach ($_POST['planneds'] as $userId => $leavePlanneds) {
        foreach ($leavePlanneds as $leaveTypeId => $plannedData) {
            $planned = $plannedData['planned'];
            $plannedId = $plannedData['planned_id'];

            if ($plannedId && is_numeric($planned)) {
                // Update the planned in the database
                $updateQuery = "
            ...
            } elseif (is_numeric($planned)) {
                // Insert new planned if it doesn't exist
                $insertQuery = "
                    INSERT INTO amountoftimeoff (user_id, time_off_type, planned)
                    VALUES (:userId, :leaveTypeId, :planned)
                ";
...
$columns = [];
    foreach ($leaveTypes as $leaveType) {
        ...
        $columns[] = "MAX(CASE WHEN a.time_off_type = $leaveTypeId THEN a.planned END) AS `" . htmlspecialchars($leaveName) . " (Days)`";
        $columns[] = "MAX(CASE WHEN a.time_off_type = $leaveTypeId THEN a.id END) AS `" . htmlspecialchars($leaveName) . " (Planned ID)`";
    }
...
$prep = $con->prepare($planned_count);
    ...
    <div class="content">
        ...
        <div class="requestBody">
            <div class="requestTableHead">
                <h5>
                    Employee Current Leave planner
                </h5>
            </div>
                        ...
                        <?php foreach ($results as $result): ?>
                            <tr>
                                ...
                                <?php foreach ($leaveTypes as $leaveType){ 
                                    ...
                                    $plannedField = "{$leaveName} (Days)";
                                    $plannedIdField = "{$leaveName} (Planned ID)";
                                    $planned = $result[$plannedField] ?? '0';
                                    $plannedId = $result[$plannedIdField] ?? 'N/A';
                                    ?>
                                    
                                <td>
                                <input type="hidden" name="planneds[<?= $result['Code'] ?>][<?= $leaveType['id'] ?>][planned_id]" id="" value="<?= $plannedId  ?>">
                                <input type="number" step='0.1' name="planneds[<?= $result['Code'] ?>][<?= $leaveType['id'] ?>][planned]" id="" value="<?= $planned ?>">
                                </td>
                                <?php } ?>
                            </tr>
                        <?php endforeach; ?>
